...
生成後に実際の音声の長さを測り、動画尺を超えていたらナレーションを縮めて再生成する。
**最大3回まで繰り返す。**
...
- **音声秒数 > 動画合計秒数** → 以下の手順でナレーションを縮めて 5-1 からやり直す:
  1. 新しい目標文字数を計算する: `新目標 = floor(現在の文字数 × 動画秒数 / 音声秒数 × 0.9)`
  2. ナレーション本文を新目標文字数以内に縮める（内容の流れ・です/ます調を維持しつつ削る）
  3. `テロップとナレーション.md` のナレーション行を更新する
  4. 縮めたテキストで 5-1 の生成コマンドを再実行する
  5. 5-2 → 5-3 を繰り返す（最大3回）
